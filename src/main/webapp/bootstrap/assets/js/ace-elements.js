...
(function($ , undefined) {
	$.fn.ace_wysiwyg = function($options , undefined) {
		...
		var button_defaults =
		{
			...
			'undo' : {
				icon : 'fa fa-undo',
				title : '取消操作 (Ctrl/Cmd+Z)'
			},
			'redo' : {
				icon : 'fa fa-repeat',
				title : '重新操作 (Ctrl/Cmd+Y)'
			},
			'viewSource' : {
				icon : 'fa fa-code',
				title : '查看源码'
		...
		this.each(function() {
			var toolbar = ' <div class="wysiwyg-toolbar btn-toolbar left"> <div class="btn-group"> ';
			...
			for(var tb in toolbar_buttons) if(toolbar_buttons.hasOwnProperty(tb)) {
				...
				switch(button.name) {
					...
					case 'insertImage':
						toolbar += ' <div class="btn-group"> <a class="btn btn-sm '+className+' " title="'+button.title+'" onclick="'+button.button_text+'" '+'><i class="'+ ace.vars['icon'] + button.icon+'"></i></a> ';
						toolbar += ' </div>';
					break;

					case 'foreColor':
					case 'backColor':
						toolbar += ' <div class="btn-group"><select class="hide wysiwyg_colorpicker" title="'+button.title+'"> ';
						$.each(button.values, function (_, color) {
                            toolbar += ' <option value="' + color + '">' + color + '</option> ';
                        });
						toolbar += ' </select> ';
						toolbar += ' <input style="display:none;" disabled class="hide" type="text" data-edit="'+button.name+'" /> </div>';
					break;
